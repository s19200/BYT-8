...
public class PersonClientsTest {
	...
	public void testClients() throws IOException {
		...
		out = new StringWriter();
		Client2.display(out, bobSmith);
		assertEquals("Smith, Bob", out.toString());

		out = new StringWriter();
		Client2.display(out, jennyJJones);
		assertEquals("Jones, Jenny J", out.toString());

		assertEquals("Smith, Bob", Client1.toString(bobSmith));
		assertEquals("Jones, Jenny J", Client1.toString(jennyJJones));
	}
}
